...
func TestTokenSourceCurrentText(t *testing.T) {
	ts := NewTokenSource("foo = $VALUE$")
	if got := ts.CurrentText(); got != "foo" {
		t.Fatalf("current text = %q, want %q", got, "foo")
	}
	ts.Bump()
	if got := ts.CurrentText(); got != "=" {
		t.Fatalf("current text = %q, want %q", got, "=")
	}
	ts.Bump()
	if got := ts.CurrentText(); got != "$VALUE$" {
		t.Fatalf("current text = %q, want %q", got, "$VALUE$")
	}
}
...
func TestTokenSourceReLexesParameterAndRestoresNormalMode(t *testing.T) {
	...
	if ts.Current() != syntax.Identifier {
		t.Fatalf("normal parameter = %s, want Identifier", ts.Current())
	}
	ts.ReLex(lexer.ReLexParameter)
	want := []syntax.SyntaxKind{
		syntax.Dollar,
		syntax.ParameterName,
		syntax.Pipe,
		syntax.ParameterArgument,
		syntax.Dollar,
		syntax.Identifier,
	}
	for _, kind := range want {
